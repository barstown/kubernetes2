...
{{ .contents | indent 8 }}
  ...
  {{- end }}
  {{- end }}
  {{- if not (empty .Values.extraManifests) }}
  extraManifests:
    {{- toYaml .Values.extraManifests | nindent 4 }}
  {{- end }}
  {{- if eq .MachineType "controlplane" }}
  allowSchedulingOnControlPlanes: {{ .Values.allowSchedulingOnControlPlanes | default "true" }}
  controllerManager:
    extraArgs:
